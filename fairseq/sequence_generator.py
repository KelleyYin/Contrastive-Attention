...
class SequenceGenerator(object):
    ...
    def _generate(self, src_tokens, src_lengths, beam_size=None, maxlen=None, prefix_tokens=None):
        ...
        # for sent in range(len(finalized)):
        #     tgt_tokens = []
        #     for beam_sent in finalized[sent]:
        #         tgt_tokens.append(beam_sent["tokens"])
        #     print(tgt_tokens)
        #
        #     tgt_batch = data_utils.collate_tokens(tgt_tokens, self.pad, self.eos,
    ...
    def _decode_one(self, tokens, model, encoder_out, incremental_states, log_probs):
        with torch.no_grad():
            ...
            if attn is not None:
                attn = attn.sum(dim=1) / 8
                attn = attn[:, -1, :]
        probs = model.get_normalized_probs(decoder_out, log_probs=log_probs)
        oppo_probs = torch.log(F.softmax(torch.neg(decoder_out[1].float()), dim=-1))
        total_probs = probs + 0.2 * oppo_probs

        return total_probs, attn
