...
def train(args, trainer, task, epoch_itr):
    ...
    for i, sample in enumerate(progress, start=epoch_itr.iterations_in_epoch):
        ...
        if args.save_interval_updates > 0 and num_updates % args.save_interval_updates == 0:
            valid_losses, valid_state = validate(args, trainer, task, epoch_itr, [first_valid])
            save_checkpoint(args, trainer, epoch_itr, valid_losses[0], valid_state)
    ...
    for k in ['train_loss', 'train_nll_loss', 'wps', 'ups', 'wpb', 'bsz', 'clip', "acc"]:
...
def get_training_stats(trainer):
    ...
    stats["acc"] = round(trainer.get_meter("acc").acc_avg, 2)
...
def validate(args, trainer, task, epoch_itr, subsets):
    ...
    for subset in subsets:
        ...
        for k in ['valid_loss', 'valid_nll_loss', "acc"]:
            meter = trainer.get_meter(k)
            if meter is not None:
                meter.reset()
        extra_meters = collections.defaultdict(lambda: AverageMeter())
